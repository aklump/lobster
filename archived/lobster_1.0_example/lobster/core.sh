...
#
# Assure that all necessary project scope folders exist
#
#
function lobster_init() {
  local framework="$LOBSTER/framework"
  local location="$LOBSTER_PROJECT"
  lobster_install_framework "$framework" "$location"
}
...
function lobster_install_framework() {
  ...
  lobster_progress "Installing framework: $from into $to" ok
  rsync -av "$from/" "$to/" --delete --dry-run 

  # # Create these dirs in the project
  # create_dirs=("$LOBSTER_PROJECT/files/tmp" "$LOBSTER_PROJECT/hooks" "$LOBSTER_PROJECT/docs")
  # for file in "${create_dirs[@]}"; do
  #   mkdir -p "$file"
  #   if [[ -d "$file" ]]; then
  #     lobster_progress "Directory created: $file" ok
  #   else
  #     lobster_progress "Unable to create directory: $file" error
  #   fi
  # done
}
...
function lobster_deliver() {
  ...
  if [[ "${LOBSTER_ARGS[0]}" ]]; then
    lobster_alert "\"$item\" is not a valid command." error
  fi
...
function lobster_menu_item() {
  ...
  lobster_alert "Cannot add menu item \"$1\"" error
  return 1
}
